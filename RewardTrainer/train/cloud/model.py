...
class RewardHead(nn.Module):
    ...
    def __init__(self, cfg: PretrainedConfig, n_labels: int, dtype: torch.dtype = None):
        super().__init__()
        # 如果指定了 dtype，则使用指定的 dtype 初始化层
        self.dense = nn.Linear(cfg.hidden_size, cfg.hidden_size, dtype=dtype)
        # use same dropout as attention dropout
        self.dropout = nn.Dropout(cfg.attention_dropout)
        self.out_proj = nn.Linear(cfg.hidden_size, n_labels, dtype=dtype)
...
class CLoudRewardModel(PreTrainedModel):
    ...
    def __init__(self, config, pretrained_reward_base_model=None):
        ...
        # === 关键修改：获取基础模型的 dtype，确保 reward_head 使用相同的 dtype ===
        try:
            base_dtype = next(self.reward_base_model.parameters()).dtype
        except StopIteration:
            base_dtype = None  # 如果模型没有参数（不太可能），使用默认值
        
        self.reward_head = RewardHead(self.reward_base_model.config, 1, dtype=base_dtype)
        # =========================================================================

        self._no_split_modules = self.reward_base_model._no_split_modules
    
    def _init_weights(self, module):
        """Initialize the weights - 用于 FSDP 的参数初始化"""
        if hasattr(self.reward_base_model, '_init_weights'):
            self.reward_base_model._init_weights(module)
        # 对于 RewardHead 中的 Linear 层
