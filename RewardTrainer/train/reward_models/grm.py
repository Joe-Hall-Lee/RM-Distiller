...
class ValueHead(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.dropout = nn.Dropout(
            summary_dropout_prob) if summary_dropout_prob else nn.Identity()
        ...
        if self.layer_type == "linear":
            ...
            for i in range(num_layers):
                module_lis.extend(
                    [nn.Linear(input_neurons, num_neurons), nn.ReLU()])
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hasattr(self.v_head.summary, "weight") and last_hidden_state.device != self.v_head.summary.weight.device:
            ...
            last_hidden_state = last_hidden_state.to(
                self.v_head.summary[0].weight.device)
        ...
        value = self.v_head(
            last_hidden_state).squeeze(-1)[torch.arange(len(last_hidden_state)), last_index]
        return value
