...
class ValueHead(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        # 获取基础模型的 dtype，默认设为 bfloat16
        self.dtype = kwargs.get("torch_dtype", torch.bfloat16)

        if self.layer_type == 'linear':
            # 显式指定线性层的 dtype
            self.summary = nn.Linear(hidden_size, num_output, dtype=self.dtype)
        else:
            module_lis = []
            ...
            for i in range(num_layers):
                # 每一层都指定 dtype
                module_lis.extend([
                    nn.Linear(input_neurons, num_neurons, dtype=self.dtype), 
                    nn.ReLU()
                ])
                input_neurons = num_neurons
            # 最后一层也指定 dtype
            module_lis.append(nn.Linear(num_neurons, num_output, dtype=self.dtype))
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        v_head_kwargs["torch_dtype"] = pretrained_model.dtype
    ...
    def forward(self, input_ids=None, past_key_values=None, attention_mask=None, **kwargs):
        ...
        if (valid_token_count == 0).any():
            raise ValueError("At least one sample in the batch has no valid tokens (attention_mask all zeros)")
        ...
        # 检查 hidden_states 的形状
        if last_hidden_state.shape[1] == 0:
            raise ValueError("Sequence length of hidden_states is 0, check input_ids and attention_mask")

        last_index = attention_mask.sum(dim=-1) - 1
        if (last_index < 0).any():
            raise ValueError("Invalid last_index computed from attention_mask")
