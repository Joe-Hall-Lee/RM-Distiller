...
MTBENCH_V2 = {
    "name": "pair-v2",
    "type": "pairwise",
    "system_prompt": prompt_v2_gemini,
...
def batch_get_ratings_vllm(dataset_formatted, vllm_model_dict, model_modifier, is_ties=False):
    """批量获取所有评分"""

    # 收集所有需要评分的 prompts
    all_prompts = []
    all_prompt_ids = []
    sample_map = []  # 记录 (sample_idx, answer_idx)

    for sample_idx, batch in enumerate(dataset_formatted):
        question_text = batch["prompt"]
        all_answers_text = batch["answers"]

        for answer_idx, answer_text in enumerate(all_answers_text):
            # 复用原来的 prompt 格式
            if is_ties:
                user_prompt = ratings_prompt_ties.format(
                    prompt=question_text, completion=answer_text
                )
            else:
                user_prompt = ratings_prompt.format(
                    prompt=question_text, completion=answer_text
                )
            ...
            # 格式化
            tokenizer = vllm_model_dict["tokenizer"]
            optional_chat_template = vllm_model_dict.get("chat_template", None)
            ...
            sample_map.append((sample_idx, answer_idx))
    logger = logging.getLogger(__name__)
    # 批量推理
    logger.info(f"Batch generating {len(all_prompt_ids)} ratings...")
    model = vllm_model_dict["model"]
    sampling_params = vllm_model_dict["sampling_params"]

    outputs = model.generate(
        prompt_token_ids=all_prompt_ids,  # 传入所有 prompt_ids
        sampling_params=sampling_params
    )

    # 解析结果
    ratings_per_sample = [[] for _ in range(len(dataset_formatted))]

    for (sample_idx, answer_idx), output in zip(sample_map, outputs):
        ...
        ratings_per_sample[sample_idx].append(parsed_rating)

    # 计算每个样本的分数
    results = []
    for ratings in ratings_per_sample:
        if is_ties:
            results.append(ratings)  # Ties 返回原始评分
        else:
