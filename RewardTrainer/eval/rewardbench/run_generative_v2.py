...
from .utils import load_eval_dataset_multi, process_single_model
from .generative_v2 import (
...
def get_args():
    ...
    parser.add_argument("--dataset", type=str,
                        default="data/eval/rewardbench_v2.jsonl", help="path to huggingface dataset")
    ...
    parser.add_argument("--vllm_gpu_util", type=float,
                        default=1.0, help="gpu utilization for vllm")
    ...
    parser.add_argument(
        "--num_threads", type=int, default=64, help="number of threads to use for parallel processing of examples"
    )
    parser.add_argument(
...
def main():
    ...
    dataset = load_eval_dataset_multi(
        ...
        tokenizer=None,
        logger=logger,
        keep_columns=["texts_chosen", "texts_rejected",
                      "id", "subset", "num_correct"],
        max_turns=4,
    )

    # copy id for saving, then remove
    # save ties_ids separately because they are needed for processing ties results
    ties_ids = dataset.filter(
        lambda example: example["subset"] == "Ties")["id"]
    ...
    if is_api_models:
        ...
        with ThreadPoolExecutor(max_workers=args.num_threads) as executor:
            ...
            with ThreadPoolExecutor(max_workers=args.num_threads) as executor:
                ...
                future_to_index = {executor.submit(
                    judge_fn, x): i for i, x in enumerate(dataset)}
            ...
            with ThreadPoolExecutor(max_workers=args.num_threads) as executor:
                # Submit all tasks and hold their futures in a list
                future_to_index = {executor.submit(
                    judge_fn_ties, x): i for i, x in enumerate(ties_dataset)}
        ...
        def format_judgements(batch, optional_chat_template=None):
            ...
            if optional_chat_template is not None:
                ...
                tokenized_prompt = tokenizer(
                    prompt, add_special_tokens=False, return_length=True)
        ...
        def get_vllm_judgement(batch, is_ties=False):
            ...
            if not args.score_w_ratings and not is_ties:
                ...
                mult_turn = batch["mult_turn"] if "mult_turn" in batch else (
                    len(batch["texts_chosen"]) > 2)
                prompt = batch["texts_chosen"][0][0]["content"]
                ...
                if "answers" in batch:
                    ...
                    all_answers = [batch["texts_chosen"]] + \
                        batch["texts_rejected"]
                    all_answers_text = [ans[1]["content"]
        ...
        if args.score_w_ratings:
            ...
            logger.info(
                "*** Run inference on non-ties subsets with ratings ***")
            dataset_formatted = dataset.map(
                format_ratings, fn_kwargs={"is_ties": False})
            results = []
            ...
            logger.info(
                "*** Run inference on non-ties subsets with 4-way comparison ***")
            if args.chat_template is not None:
                chat_template = get_conv_template(args.chat_template)
            else:
                chat_template = None
            ...
            results = [process_shuffled(w, s)
                       for w, s in zip(winners, shuffle_position)]
        ...
        ties_dataset_formatted = ties_dataset.map(
            format_ratings, fn_kwargs={"is_ties": True})
        results_ties = []
        results_ties = batch_get_ratings_vllm(
            ties_dataset_formatted,
            vllm_model_dict,
            model_modifier,
            is_ties=True
        )
