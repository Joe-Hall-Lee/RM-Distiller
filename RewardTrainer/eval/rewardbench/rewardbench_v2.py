...
from .utils import load_bon_dataset_v2, reroll_and_score_dataset, process_single_model
...
def get_args():
    ...
    parser.add_argument("--revision", type=str, default=None,
                        help="revision of model to load")
    parser.add_argument(
        "--dataset", type=str, default="data/eval/rewardbench_v2.jsonl", help="dataset, local or from huggingface"
    )
    parser.add_argument("--tokenizer", type=str, default=None,
                        help="path to non-matching tokenizer to model")
    parser.add_argument("--chat_template", type=str,
                        default="tulu", help="path to chat template")
    parser.add_argument(
    ...
    parser.add_argument("--debug", action="store_true",
                        help="Debug on small set of examples")
    parser.add_argument(
...
def main():
    ...
    # todo will be needed to add PairRM and SteamSHP
    ...
    if pipeline_builder == pipeline:
        logger.info(
            "*** Running forward pass via built in pipeline abstraction ***")
        # this setup can be optimized slightly with one pipeline call
        # prepare for inference
        reward_pipe = accelerator.prepare(reward_pipe)

        results_rej = reward_pipe(
            dataset["text_rejected"], **reward_pipeline_kwargs)
        results_cho = reward_pipe(
            dataset["text_chosen"], **reward_pipeline_kwargs)
        ...
        results = [1 if chosen > rejected else 0 for chosen,
                   rejected in zip(scores_chosen, scores_rejected)]
    ...
    chat_template = args.chat_template if not check_tokenizer_chat_template(
        tokenizer) else "tokenizer"
    ...
    for subset in present_subsets:
        ...
        if subset.lower() == "ties":
            ...
            ties_indices = [i for i, s in enumerate(
                out_dataset["subset"]) if s == "ties"]
            out_dataset_df = out_dataset.to_pandas()
            for i, ties_idx in enumerate(ties_indices):
                out_dataset_df.at[ties_idx,
                                  "results"] = ties_subset_with_results["results"][i]
