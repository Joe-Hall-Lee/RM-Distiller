...
class DPOInference:
    ...
    def tokenize_row(self, feature) -> Dict:
        ...
        if not self.is_encoder_decoder:
            ...
            if not isinstance(prompt, str):
                raise ValueError(
                    f"prompt should be an str but got {type(prompt)}")
            prompt_tokens = self.tokenizer(prompt, add_special_tokens=False)
            prompt_tokens = {f"prompt_{k}": v for k,
                             v in prompt_tokens.items()}
    ...
    def build_tokenized_answer(self, prompt, answer):
        ...
        answer_attention_mask = full_tokenized["attention_mask"][len(
            prompt_input_ids):]
    ...
    def inference_step(self, batch, ref_free: bool = False) -> list:
        ...
        if not ref_free:
            ...
            chosen_logratios = policy_chosen_logps.detach().cpu() - \
                ref_chosen_logps.detach().cpu()
            rejected_logratios = policy_rejected_logps.detach().cpu() - \
                ref_rejected_logps.detach().cpu()
        else:
            chosen_logratios = policy_chosen_logps.detach().cpu()
            rejected_logratios = policy_rejected_logps.detach().cpu()
            ...
            if k.startswith("rejected") and isinstance(batch[k], torch.Tensor):
                ...
                concatenated_batch[concatenated_key] = torch.cat(
                    (
                        ...
                        pad_to_length(batch[k], max_length,
                                      pad_value=pad_value),
                    ),
                    dim=0,
                ).to(device=device)
